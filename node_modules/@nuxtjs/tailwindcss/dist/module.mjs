...
import { useNuxt, tryResolveModule, createResolver, addTemplate, findPath, resolveAlias, isNuxt3, addDevServerHandler, isNuxt2, useLogger, defineNuxtModule, resolvePath, getNuxtVersion, installModule, addVitePlugin } from '@nuxt/kit';
...
const resolveContentPaths = (srcDir, nuxt = useNuxt()) => {
  ...
  Array.from(/* @__PURE__ */ new Set(["vue", ...nuxt.options.extensions]));
  const sfcExtensions = extensionFormat(Array.from(/* @__PURE__ */ new Set(["vue", ...nuxt.options.extensions])).map((e) => e.replace(/^\.*/, "")));
...
const resolveViewerConfig = (config) => resolveBoolObj(config, { endpoint: "_tailwind" });
...
async function setupViewer(twConfig, config, nuxt = useNuxt()) {
